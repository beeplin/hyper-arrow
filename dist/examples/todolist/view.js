...
export function view(/**@type {ToDoListState}*/ s) {
    ...
    return div({ id: 'root' }, [
        ...
        ul({ id: 'list', style: 'padding: 0', cacheRemovedChildren: true }, () => s.getFilteredReversedList().map((_, i) => li({ id: () => 'li-' + item(i).id, class: 'item-container' }, [
            button({
                id: () => 'up-' + item(i).id,
                class: 'item-up',
                innerHTML: '⇧',
            ...
            () => s.isEditing(item(i).id)
                ? input({
                    ...
                    onKeyDown(/**@type {any}*/ e) {
                        if (e.keyCode === 13)
                            s.update(item(i).id, s.editInput);
                    },
            ...
            button({
                id: () => (s.isEditing(item(i).id) ? 'cancel' : 'delete') + '-' + item(i).id,
