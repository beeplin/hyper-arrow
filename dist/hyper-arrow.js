...
function createREl(/**@type {VEl}*/ vel) {
    ...
    (_b = (_a = vel[PROPS])[ON_CREATE]) === null || _b === void 0 ? void 0 : _b.call(_a, el);
...
function convertVNodeToRNode(/**@type {VNode}*/ vnode, /**@type {El|Text}*/ node) {
    ...
    rnode[NODE] = node;
    const hasCache = rnode instanceof VEl &&
    ...
    return new PROXY(obj, {
        get(obj, prop) {
            ...
            if (currentFaci) {
                ...
                if (!faci2ropa.has(currentFaci))
                    faci2ropa.set(currentFaci, new WeakMap());
                const ropas = faci2ropa.get(currentFaci);
                if (ropas) {
                    if (!ropas.has(obj))
                        ropas.set(obj, new Set());
                    (_a = ropas.get(obj)) === null || _a === void 0 ? void 0 : _a.add(prop);
                }
                // collect current FACI for current ROPA, only for debugging purposes
                if (!ropa2faci.has(obj))
                    ropa2faci.set(obj, OBJECT.create(null));
        ...
        set(obj, prop, newValue) {
            ...
            for (const [faci, ropas] of faci2ropa.entries())
                if ((_a = ropas.get(obj)) === null || _a === void 0 ? void 0 : _a.has(prop)) {
        ...
        deleteProperty(obj, prop) {
            ...
            // console.log('--delete--')
            // console.log(obj, prop)
            for (const ropas of faci2ropa.values())
                (_a = ropas.get(obj)) === null || _a === void 0 ? void 0 : _a.delete(prop);
            (_b = ropa2faci.get(obj)) === null || _b === void 0 ? true : delete _b[prop];
            return result;
        },
    });
}
function removeFacisInRNodeFromDeps(/**@type {RNode}*/ rnode) {
    for (const faci of faci2ropa.keys())
        if (faciIsInRNode(faci, rnode))
            faci2ropa.delete(faci);
}
function faciIsInRNode(/**@type {Faci}*/ faci, /**@type {RNode}*/ rnode) {
    var _a;
    return faci[REL] && rnode[NODE].contains((_a = faci[REL]) === null || _a === void 0 ? void 0 : _a[NODE]);
}
...
/**@type {ANode}*/ newANode) {
    ...
    (_a = rel[CACHE]) === null || _a === void 0 ? true : delete _a[newRNode[PROPS].id];
}
